  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-800/80 pb-1">
        <h3 className="font-sans font-black text-[10px] tracking-widest text-slate-400 uppercase">
          SWEET GAME BOOSTERS
        </h3>
        {selectedBooster && (
          <button
            onClick={() => { playClickSound(); onSelectBooster(null); }}
            className="flex items-center gap-1 text-[9px] font-bold text-rose-400 hover:text-rose-300 transition-colors bg-rose-500/10 hover:bg-rose-500/20 px-1.5 py-0.5 rounded-md"
          >
            <X className="w-3 h-3" />
            Cancel
          </button>
        ...
        {boosters.map((booster) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center gap-1 p-1 py-1.5 rounded-xl transition-all duration-300 border select-none ${
                isSelected
                  ? 'bg-purple-900/40 border-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.3)] scale-105 ring-1 ring-white/10'
                  : isDisabled
            ...
            >
              {/* Count bubble */}
              <span className="absolute top-0.5 right-0.5 flex items-center justify-center bg-purple-600 text-[8px] font-bold font-mono h-3.5 min-w-3.5 px-0.5 rounded-full border border-purple-400 shadow-md">
                {booster.count}
              </span>

              {/* Icon */}
              <div className={`p-1 rounded-lg ${isSelected ? 'bg-purple-500/20' : 'bg-slate-900/50'}`}>
                {getIcon(booster.id)}
              </div>
      ...
      {selectedBooster && (
        <div className="bg-purple-950/30 border border-purple-900/50 rounded-xl p-1.5 text-[9px] font-semibold text-center text-purple-300 animate-pulse">
